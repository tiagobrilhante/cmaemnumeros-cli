              ...
              <v-col class="text-center">
                <v-row>
                  ...
                  <v-col class="text-left">
                    <v-btn class="primary" @click="changeYear('up')">
              ...
              <v-col v-for="categoria in selectedSecao.categoria" :key="categoria.id" cols="3">
                <v-card class="pl-2 pr-2 pb-1" elevation="11">
                  ...
                  <v-alert v-for="indicador in categoria.indicadores" :key="indicador.id"
                    ...
                    <span v-if="inputsUpdate[indicador.id]">Atualizado em: {{ formatDateTime(inputsUpdate[indicador.id]) }} </span>
...
export default {
  ...
  methods: {
    ...
    getIndicadoresVigentes (secao, mes, ano, tipo, objeto) {
      if (tipo === 'consulta') {
        let objetoParaEnvio = {}
        objetoParaEnvio['mes'] = mes
        objetoParaEnvio['ano'] = ano
        objetoParaEnvio['secao_id'] = secao
        try {
          this.$http.post('valorindicador/corrente', objetoParaEnvio)
            .then(response => {
              const indicadoresValores = response.data
              console.log('indicadoresValores - indicadores vigentes')
              console.log(indicadoresValores)
              console.log('indicadoresValores')
    ...
    getCurrentDate () {
      ...
      // Define o mês e ano, usando `.getMonth()` e `.getFullYear()`
      const months = [
        'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho',
        'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'
      ]
      ...
      this.mesCorrente = months[currentDate.getMonth()] // Retorna o mês como texto
      this.anoBase = months[currentDate.getMonth()] // Retorna o mês como texto
      this.anoCorrente = currentDate.getFullYear() // Retorna o ano
    },
